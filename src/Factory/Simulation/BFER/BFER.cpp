...
{
	auto sn = Simulation::parameters::get_short_names();
...
::get_prefixes() const
{
	auto p = Simulation::parameters::get_prefixes();
...
::callback_arguments()
{
	...
	n_threads = 1;
#else
	n_threads = (n_threads == 0 && std::thread::hardware_concurrency())
	                   ? std::thread::hardware_concurrency() : 1;
#endif

	if (err_track_revert)
	{
		err_track_enable = false;
		n_threads = 1;
	}
}
...
::get_headers(std::map<std::string,header_list>& headers, const bool full) const
{
	...
	headers[p].push_back(std::make_pair("Coset approach (c)", coset ? "yes" : "no"));
	headers[p].push_back(std::make_pair("Coded monitoring", coded_monitoring ? "yes" : "no"));

	std::string enable_track = (err_track_enable) ? "on" : "off";
	headers[p].push_back(std::make_pair("Bad frames tracking", enable_track));

	std::string enable_rev_track = (err_track_revert) ? "on" : "off";
	headers[p].push_back(std::make_pair("Bad frames replay", enable_rev_track));

	if (err_track_threshold)
		headers[p].push_back(std::make_pair("Bad frames threshold", std::to_string(err_track_threshold)));

	if (err_track_enable || err_track_revert)
	{
		std::string path = err_track_path + std::string("_$noise.[src,enc,chn]");
		headers[p].push_back(std::make_pair("Bad frames base path", path));
	}

	if (src != nullptr && cdc != nullptr)
	{
		const auto bit_rate = (float)src->K / (float)cdc->N;
		// find the greatest common divisor of K and N
		auto gcd = tools::greatest_common_divisor(src->K, cdc->N);
		std::stringstream br_str;
		br_str << bit_rate << " (" << src->K/gcd << "/" << cdc->N/gcd << ")";

		headers[p].push_back(std::make_pair("Bit rate", br_str.str()));
	}

	if (src != nullptr)
		headers[p].push_back(std::make_pair("Inter frame level", std::to_string(src->n_frames)));

	if (src != nullptr) { src->get_headers(headers, full); }
	if (crc != nullptr) { crc->get_headers(headers, full); }
	if (cdc != nullptr) { cdc->get_headers(headers, full); }
	if (mdm != nullptr) { mdm->get_headers(headers, full); }
	if (chn != nullptr) { chn->get_headers(headers, full); }
	if (qnt != nullptr) { qnt->get_headers(headers, full); }

	if (mnt_er != nullptr) { mnt_er->get_headers(headers, full); }


	headers[mnt_er->get_short_name()].push_back(std::make_pair("Compute mutual info", mutinfo ? "yes" : "no"));
	if (mutinfo)
		if (mnt_mi != nullptr) { mnt_mi->get_headers(headers, full); }

	if (ter != nullptr) { ter->get_headers(headers, full); }
}
